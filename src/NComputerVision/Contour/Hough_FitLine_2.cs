...
{
    /// <summary>
    /// foamliu, 2009/02/16, 直线检测的梯度版本.
    /// 
    /// </summary>
    public class Hough_FitLine_2
    {
        private int[][] m_mat;
        ...
        public Hough_FitLine_2(int[][] mat)
        {
            // 参数的默认值
            ...
            for (int y = 0; y < m_height; y++)
            {
                for (int x = 0; x < m_width; x++)
                {
                    ...
                    if (dmag > Gradient_Threshold)
                    {
                        ...
                        theta = System.Math.Atan2(dy, dx);
                        theta = theta + System.Math.PI / 2;
                        if (theta < 0)
                            theta = 2 * System.Math.PI + theta;
                        thetaQ = Convert.ToInt32(theta / ThetaStepRadian);
                        ...
                        // 相应的累加器单元增加
                        m_ihist[dQ][thetaQ] += Convert.ToInt32(dmag);
            ...
            for (int d = 0; d < height; d++)
            {
                for (int theta = 0; theta < width; theta++)
                {
                    if (max < m_ihist[theta][d])
                    {
                        ...
                        // foamliu, 2010/05/05, 避免重复.
                        m_ihist[theta][d] = double.MinValue;
                    }
                }
            }
